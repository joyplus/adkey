...
public class Downloader {
	...
	public Downloader(String urlstr, Context context) {
		 ...
		 */
		File cacheDir = new File("/data/joy/");
		if (!cacheDir.exists())
			cacheDir.mkdirs();
	}
	...
	public class MyThread extends Thread {
		...
		public void run() {
			...
			localfile = "/data/joy/"+Const.DOWNLOADING_FILE;
			HttpURLConnection connection = null;
			RandomAccessFile randomAccessFile = null;
			InputStream inputstream = null;
			try {
				Log.d("AdBootScreen","111 Downloading ..."+compeleteSize+"  totle="+fileSize+" urlstr="+urlstr);
				URL url = new URL(urlstr);
				...
				int length = -1;
				Log.d("AdBootScreen","222 Downloading ..."+compeleteSize+"  totle="+fileSize);
				while ((length = inputstream.read(buffer)) != -1) {
					randomAccessFile.write(buffer, 0, length);
					compeleteSize += length;
					Log.d("AdBootScreen","Downloading ..."+compeleteSize+"  totle="+fileSize);
					if (compeleteSize == fileSize) {
						...
						File file = new File("/data/joy/"+Const.DOWNLOAD_READY_FILE);
						if(file.exists())
						{
							file.delete();
						} 
						 ...
						 */
						File filetemp = new File("/data/joy/"+Const.DOWNLOADING_FILE);
						if(filetemp.exists())
						{
							File filedone = new File("/data/joy/"+Const.DOWNLOAD_PLAY_FILE+Util.ExternalName);
							if(filedone.exists()){
								filedone.delete();
							}
